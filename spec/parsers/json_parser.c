      ...
      { // Capture Table
        ...
        { // Sequence with 2 patterns
          ...
          { // Constant Capture
            ...
            pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[2], 0, 0); // "json"
          }
          if (parser->success) {
            { // Choice
              parse_value(parser);
      ...
      if (parser->success) {
        { // Capture Table
          ...
          { // Sequence with 2 patterns
            ...
            { // Constant Capture
              // A constant capture matches the empty string and produces all given values
              pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[0], 0, 0); // "array"
            }
            if (parser->success) {
    ...
    if (parser->success) {
      { // Capture Table
        ...
        { // Sequence with 2 patterns
          ...
          { // Constant Capture
            // A constant capture matches the empty string and produces all given values
            pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[1], 0, 0); // "boolean"
          }
          if (parser->success) {
            { // Constant Capture
              // A constant capture matches the empty string and produces all given values
              pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[8], 0, 0); // false
            }
            if (!parser->success) {
              RESTORE_POSITION(parser, pos);
            }
...
static bool parse_member(Parser *parser) {
  ...
  { // Capture Table
    ...
    { // Sequence with 6 patterns
      ...
      { // Constant Capture
        // A constant capture matches the empty string and produces all given values
        pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[3], 0, 0); // "member"
      }
      if (parser->success) {
        parse_string(parser);
        if (parser->success) {
  ...
  { // Sequence with 2 patterns
    ...
    if (parser->success) {
      { // Capture Table
        ...
        { // Constant Capture
          // A constant capture matches the empty string and produces all given values
          pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[4], 0, 0); // "null"
        }
...
static bool parse_number(Parser *parser) {
  ...
  { // Capture Table
    ...
    { // Sequence with 2 patterns
      ...
      { // Constant Capture
        // A constant capture matches the empty string and produces all given values
        pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[5], 0, 0); // "number"
      }
      if (parser->success) {
        { // Capture
      ...
      if (parser->success) {
        { // Capture Table
          ...
          { // Sequence with 2 patterns
            ...
            { // Constant Capture
              // A constant capture matches the empty string and produces all given values
              pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[6], 0, 0); // "object"
            }
            if (parser->success) {
              { // At most 1 repetitions
                size_t rep_count = 0;
    ...
    if (parser->success) {
      { // Capture Table
        ...
        { // Sequence with 2 patterns
          ...
          { // Constant Capture
            // A constant capture matches the empty string and produces all given values
            pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[7], 0, 0); // "string"
          }
          if (parser->success) {
            { // Capture
              size_t start_pos = parser->pos;
      ...
      { // Capture Table
        ...
        { // Sequence with 2 patterns
          ...
          if (parser->success) {
            { // Constant Capture
              // A constant capture matches the empty string and produces all given values
              pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[9], 0, 0); // true
            }
            if (!parser->success) {
              RESTORE_POSITION(parser, pos);
            }
...
#define PGEN_PARSER_MT "pgen.json_parser"
...
static int l_json_parser_gc(lua_State *L) {
  json_parser_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_json_parser(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_json_parser_gc);
