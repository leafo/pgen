...
#define PGEN_PARSER_MT "pgen.transform_bad"
...
static int l_transform_bad_gc(lua_State *L) {
  transform_bad_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_transform_bad(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_transform_bad_gc);
