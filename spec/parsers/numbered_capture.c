  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (parser->pos < parser->input_len) {
      ...
      case 57:
        ...
        break;
      case 49:
        pgen_dispatch_mask = 0x0000000000000601ULL;
        break;
      default:
...
static bool parse_test10(Parser *parser) {
  ...
  { // Numbered Capture (select 2)
    ...
    { // Sequence with 3 patterns
      ...
      { // Capture Group "a"
        ...
        { // Constant Capture
          ...
          pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[0], 0, 0); // "one"
        }
      ...
      if (parser->success) {
        { // Constant Capture
          // A constant capture matches the empty string and produces all given values
          pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[2], 0, 0); // "two"
        }
        if (parser->success) {
          { // Capture Group "b"
            size_t cg_cap_start = parser->cap_len;
            ...
            { // Constant Capture
              // A constant capture matches the empty string and produces all given values
              pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[1], 0, 0); // "three"
            }
    ...
    if (parser->success) {
      pgen_cap_select(parser, cn_cap_start, 2);
    }
  }
...
static bool parse_test3(Parser *parser) {
  ...
  { // Numbered Capture (select 3)
    ...
    if (parser->success) {
      pgen_cap_select(parser, cn_cap_start, 3);
    }
  }
...
static bool parse_test5(Parser *parser) {
  ...
  { // Numbered Capture (select 5)
    ...
    if (parser->success) {
      pgen_cap_select(parser, cn_cap_start, 5);
    }
  }
...
#define PGEN_PARSER_MT "pgen.numbered_capture"
...
static int l_numbered_capture_gc(lua_State *L) {
  numbered_capture_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_numbered_capture(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_numbered_capture_gc);
