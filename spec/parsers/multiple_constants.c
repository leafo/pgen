...
#define PGEN_PARSER_MT "pgen.multiple_constants"
...
static int l_multiple_constants_gc(lua_State *L) {
  multiple_constants_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_multiple_constants(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_multiple_constants_gc);
