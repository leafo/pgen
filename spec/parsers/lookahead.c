...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "positive", start);
#endif
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "positive", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
...
#define PGEN_PARSER_MT "pgen.lookahead"
...
static int l_lookahead_gc(lua_State *L) {
  lookahead_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_lookahead(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_lookahead_gc);
...
int luaopen_lookahead(lua_State *L) {
  ...
  lua_newtable(L);
  luaL_register(L, NULL, lookahead_module);
  return 1;
}
#endif
...
gcc -shared -o lookahead.so -fPIC lookahead.c `pkg-config --cflags --libs lua5.1`
