...
// dispatch_choice - generated parser
...
static const char __cmt_code_0[] = "return function(value)\n    return value:upper()\n  end";
static const char __cmt_code_1[] = "local subject, pos, value = ...\n    if subject:sub(pos, pos) == \"x\" then\n      return pos, value\n    end\n    return false\n  ";

static int __cmt_refs[2];
...
static bool parse_at_eof(Parser *parser);
static bool parse_disjoint(Parser *parser);
static bool parse_labeled(Parser *parser);
static bool parse_lrec(Parser *parser);
static bool parse_nullable(Parser *parser);
static bool parse_overlap(Parser *parser);
static bool parse_recursive(Parser *parser);
static bool parse_trailing(Parser *parser);
static bool parse_transform(Parser *parser);
static bool parse_unknown(Parser *parser);
...
static bool parse_test(Parser *parser) {
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label) {
      PGEN_RECORD_FURTHEST(parser);
#ifdef PGEN_ERRORS
      if (pgen_dispatch_mask != 0x00000000000001ffULL) {
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 3))) {
      ...
      { // Constant Capture
        ...
        pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[2], 0, 0); // "eof"
      }
    }
    if (!parser->success && !parser->throw_label) {
      PGEN_RECORD_FURTHEST(parser);
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "at_eof", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "at_eof", parser->pos);
  }
#endif
...
static bool parse_disjoint(Parser *parser) {
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "disjoint", start);
#endif
  ...
  { // FIRST-byte dispatched ordered choice
        ...
        { // Sequence with 2 patterns
          ...
          { // Match literal "apple"
            if (parser->pos + 5 <= parser->input_len &&
                memcmp(parser->input + parser->pos, "apple", 5) == 0) {
              parser->pos += 5;
            } else {
#ifdef PGEN_ERRORS
              sprintf(parser->error_message, "Expected `"
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 1))) {
      ...
      { // Capture
        size_t start_pos = parser->pos;
        { // Match literal "banana"
          if (parser->pos + 6 <= parser->input_len &&
              memcmp(parser->input + parser->pos, "banana", 6) == 0) {
            parser->pos += 6;
          } else {
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "Expected `"
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 2))) {
      ...
      { // Capture
        size_t start_pos = parser->pos;
        { // Match literal "cherry"
          if (parser->pos + 6 <= parser->input_len &&
              memcmp(parser->input + parser->pos, "cherry", 6) == 0) {
            parser->pos += 6;
          } else {
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "Expected `"
      ...
      { // Capture
        ...
        { // Match literal "date"
          if (parser->pos + 4 <= parser->input_len &&
              memcmp(parser->input + parser->pos, "date", 4) == 0) {
            parser->pos += 4;
          } else {
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "Expected `"
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "disjoint", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "disjoint", parser->pos);
  }
#endif
...
static bool parse_labeled(Parser *parser) {
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "labeled", start);
#endif
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 0))) {
      parser->success = true;
      { // Throw labeled failure: boom
        parser->success = false;
        parser->throw_label = "boom";
        parser->throw_pos = parser->pos;
#ifdef PGEN_ERRORS
        sprintf(parser->error_message, "boom"
                                       " at position %zu",
                parser->pos + 1);
#endif
      }
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "labeled", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "labeled", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "lrec", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "lrec", parser->pos);
  }
#endif
...
static bool parse_nullable(Parser *parser) {
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "nullable", start);
#endif
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 1))) {
      ...
      { // Constant Capture
        // A constant capture matches the empty string and produces all given values
        pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[1], 0, 0); // "empty"
      }
    }
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 2))) {
      if ((pgen_dispatch_mask & 0x0000000000000003ULL) != 0x0000000000000003ULL) {
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "nullable", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "nullable", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "overlap", start);
#endif
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "overlap", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "overlap", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "recursive", start);
#endif
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 1))) {
      ...
      { // Capture
        ...
        { // Match literal "bb"
          if (parser->pos + 2 <= parser->input_len &&
              memcmp(parser->input + parser->pos, "bb", 2) == 0) {
            parser->pos += 2;
          } else {
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "Expected `"
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 2))) {
      ...
      { // Capture
        ...
        { // Match literal "cc"
          if (parser->pos + 2 <= parser->input_len &&
              memcmp(parser->input + parser->pos, "cc", 2) == 0) {
            parser->pos += 2;
          } else {
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "Expected `"
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 3))) {
      ...
      { // Capture
        ...
        { // Match literal "dd"
          if (parser->pos + 2 <= parser->input_len &&
              memcmp(parser->input + parser->pos, "dd", 2) == 0) {
            parser->pos += 2;
          } else {
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "Expected `"
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "recursive", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "recursive", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "trailing", start);
#endif
    ...
    { // FIRST-byte dispatched ordered choice
      ...
      if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 0))) {
        ...
        { // Capture
          ...
          { // Match literal "aa"
            if (parser->pos + 2 <= parser->input_len &&
                memcmp(parser->input + parser->pos, "aa", 2) == 0) {
              parser->pos += 2;
            } else {
#ifdef PGEN_ERRORS
              sprintf(parser->error_message, "Expected `"
      ...
      if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 3))) {
        ...
        { // Capture
          size_t start_pos = parser->pos;
          { // Match single character "d"
            if (parser->pos < parser->input_len &&
                parser->input[parser->pos] == 100) {
              parser->pos++;
            } else {
#ifdef PGEN_ERRORS
              sprintf(parser->error_message, "Expected character `"
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "trailing", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "trailing", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "transform", start);
#endif
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "transform", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "transform", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "unknown", start);
#endif
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 0))) {
      ...
      { // Match-time capture (Cmt id=1)
        ...
        { // Constant Capture
          // A constant capture matches the empty string and produces all given values
          pgen_cap_push(parser, PGEN_CAP_CONST, __const_refs[0], 0, 0); // "dynamic"
        }

        if (parser->success) {
          pgen_run_cmt(parser, __cmt_refs[1], cmt_start_pos, cmt_cap_base, cmt_top_base);
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "unknown", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "unknown", parser->pos);
  }
#endif
...
#define PGEN_PARSER_MT "pgen.dispatch_choice"
...
// cannot leak them.
static Parser *dispatch_choice_init(const char *input, lua_State *L) {
...
static void dispatch_choice_free(Parser *parser) {
  if (parser) {
    free(parser->caps);
    parser->caps = NULL;
  }
...
static int l_dispatch_choice_gc(lua_State *L) {
  dispatch_choice_free((Parser *)lua_touserdata(L, 1));
  return 0;
}

// Lua wrapper function
static int l_dispatch_choice_parse(lua_State *L) {
  ...
  Parser *parser = dispatch_choice_init(input, L);
...
static const struct luaL_Reg dispatch_choice_module[] = {
    {"parse", l_dispatch_choice_parse}, // Expose l_parsername_parse as "parse" in Lua
    {NULL, NULL}                        // Sentinel
};
...
int luaopen_dispatch_choice(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_dispatch_choice_gc);
    lua_setfield(L, -2, "__gc");
  }
  lua_pop(L, 1);
  __const_init(L);
  __cmt_init(L);
  luaL_newlib(L, dispatch_choice_module); // Creates table and registers functions
  return 1;
}
#else
...
int luaopen_dispatch_choice(lua_State *L) {
  ...
  __cmt_init(L);
  lua_newtable(L);
  luaL_register(L, NULL, dispatch_choice_module);
  return 1;
}
#endif
...
gcc -shared -o dispatch_choice.so -fPIC dispatch_choice.c `pkg-config --cflags --libs lua5.1`

To use in Lua:
local dispatch_choice = require "dispatch_choice"
local result = dispatch_choice.parse("your input string")
*/
