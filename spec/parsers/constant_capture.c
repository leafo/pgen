...
static bool parse_string(Parser *parser) {
  ...
  { // Sequence with 3 patterns
    ...
    { // Match single character "\""
      if (parser->pos < parser->input_len && parser->input[parser->pos] == 34) {
        parser->pos++;
      } else {
#ifdef PGEN_ERRORS
        sprintf(parser->error_message,
                ...
                if (parser->success) {
                  ...
                  RESTORE_POSITION(
                      parser, pos); // Restore original position (technically
                                    // not necessary since failed pattern should
                                    // make no changes to position)
                }
              }
              if (parser->success) {
...
static const struct luaL_Reg constant_capture_module[] = {
    {"parse",
     l_constant_capture_parse}, // Expose l_parsername_parse as "parse" in Lua
    {NULL, NULL}                // Sentinel
};
...
int luaopen_constant_capture(lua_State *L) {
  luaL_newlib(L,
              constant_capture_module); // Creates table and registers functions
  return 1;
}
#else
// Lua 5.1 uses luaL_register
int luaopen_constant_capture(lua_State *L) {
  luaL_register(L, "constant_capture",
                constant_capture_module); // Registers functions in global table
                                          // (or package table)
  return 1;
}
#endif
