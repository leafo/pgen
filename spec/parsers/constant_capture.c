...
#define PGEN_PARSER_MT "pgen.constant_capture"
...
static int l_constant_capture_gc(lua_State *L) {
  constant_capture_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_constant_capture(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_constant_capture_gc);
