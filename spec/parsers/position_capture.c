...
#define PGEN_PARSER_MT "pgen.position_capture"
...
static int l_position_capture_gc(lua_State *L) {
  position_capture_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_position_capture(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_position_capture_gc);
...
int luaopen_position_capture(lua_State *L) {
  ...
  lua_newtable(L);
  luaL_register(L, NULL, position_capture_module);
  return 1;
}
#endif
