...
#define PGEN_PARSER_MT "pgen.many_captures"
...
static int l_many_captures_gc(lua_State *L) {
  many_captures_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_many_captures(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_many_captures_gc);
...
int luaopen_many_captures(lua_State *L) {
  ...
  lua_newtable(L);
  luaL_register(L, NULL, many_captures_module);
  return 1;
}
#endif
