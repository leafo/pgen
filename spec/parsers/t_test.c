...
static bool parse_basic_throw(Parser *parser);
static bool parse_choice_with_throw(Parser *parser);
static bool parse_conditional_throw(Parser *parser);
static bool parse_id(Parser *parser);
static bool parse_predicate_swallow(Parser *parser);
static bool parse_throw_after_capture(Parser *parser);
  ...
  { // FIRST-byte dispatched ordered choice
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 1))) {
      ...
      { // Sequence with 2 patterns
        ...
        if (parser->success) {
          parse_choice_with_throw(parser);
          if (!parser->success) {
            RESTORE_INPUT_POSITION(parser, pos);
          }
        }
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 2))) {
      ...
      { // Sequence with 2 patterns
        ...
        if (parser->success) {
          parse_conditional_throw(parser);
          if (!parser->success) {
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 3))) {
      ...
      { // Sequence with 2 patterns
        ...
        if (parser->success) {
          parse_throw_after_capture(parser);
          if (!parser->success) {
            RESTORE_POSITION(parser, pos);
          }
        }
    ...
    if (!parser->success && !parser->throw_label && (pgen_dispatch_mask & (1ULL << 4))) {
      ...
      { // Sequence with 2 patterns
        ...
        if (parser->success) {
          parse_predicate_swallow(parser);
          if (!parser->success) {
            RESTORE_INPUT_POSITION(parser, pos);
          }
        }
    ...
    if (!parser->success && !parser->throw_label) {
      PGEN_RECORD_FURTHEST(parser);
#ifdef PGEN_ERRORS
      if (pgen_dispatch_mask != 0x000000000000001fULL) {
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "basic_throw", start);
#endif

  { // Throw labeled failure: always_fails
    parser->success = false;
    parser->throw_label = "always_fails";
    parser->throw_pos = parser->pos;
#ifdef PGEN_ERRORS
    sprintf(parser->error_message, "always_fails"
                                   " at position %zu",
            parser->pos + 1);
#endif
  }
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "basic_throw", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "basic_throw", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "choice_with_throw", start);
#endif

  {   // Choice
    { // Match literal "match"
      if (parser->pos + 5 <= parser->input_len &&
          memcmp(parser->input + parser->pos, "match", 5) == 0) {
        parser->pos += 5;
      } else {
#ifdef PGEN_ERRORS
        sprintf(parser->error_message, "Expected `"
    ...
    if (!parser->success && !parser->throw_label) {
      parser->success = true;
      { // Throw labeled failure: expected_match
        parser->success = false;
        parser->throw_label = "expected_match";
        parser->throw_pos = parser->pos;
#ifdef PGEN_ERRORS
        sprintf(parser->error_message, "expected_match"
                                       " at position %zu",
                parser->pos + 1);
#endif
      }
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "choice_with_throw", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "choice_with_throw", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "conditional_throw", start);
#endif
          ...
          { // Sequence with 2 patterns
            ...
            if (parser->success) {
              { // Choice
                ...
                if (!parser->success && !parser->throw_label) {
                  parser->success = true;
                  { // Throw labeled failure: expected_identifier
                    parser->success = false;
                    parser->throw_label = "expected_identifier";
                    parser->throw_pos = parser->pos;
#ifdef PGEN_ERRORS
                    sprintf(parser->error_message, "expected_identifier"
                                                   " at position %zu",
                            parser->pos + 1);
#endif
                  }
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "conditional_throw", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "conditional_throw", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "id", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "id", parser->pos);
  }
#endif
...
static bool parse_predicate_swallow(Parser *parser) {
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "predicate_swallow", start);
#endif
  ...
  { // Sequence with 2 patterns
    ...
    { // Negate (only match if pattern fails)
      ...
      { // Throw labeled failure: nope
        parser->success = false;
        parser->throw_label = "nope";
        parser->throw_pos = parser->pos;
#ifdef PGEN_ERRORS
        sprintf(parser->error_message, "nope"
                                       " at position %zu",
                parser->pos + 1);
#endif
      }
    ...
    if (parser->success) {
      { // Match literal "ok"
        if (parser->pos + 2 <= parser->input_len &&
            memcmp(parser->input + parser->pos, "ok", 2) == 0) {
          parser->pos += 2;
        } else {
#ifdef PGEN_ERRORS
          sprintf(parser->error_message, "Expected `"
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "predicate_swallow", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "predicate_swallow", parser->pos);
  }
#endif
...
#ifdef PGEN_DEBUG
  fprintf(stderr, "%*sEntering rule %s at position %zu\n", (int)parser->depth, "", "throw_after_capture", start);
#endif
  ...
  { // Sequence with 2 patterns
    ...
    if (parser->success) {
      {   // Choice
        ...
        if (!parser->success && !parser->throw_label) {
          parser->success = true;
          { // Throw labeled failure: expected_exclamation
            parser->success = false;
            parser->throw_label = "expected_exclamation";
            parser->throw_pos = parser->pos;
#ifdef PGEN_ERRORS
            sprintf(parser->error_message, "expected_exclamation"
                                           " at position %zu",
                    parser->pos + 1);
#endif
          }
...
#ifdef PGEN_DEBUG
  if (parser->success) {
    fprintf(stderr, "%*sRule %s matched range: %zu-%zu\n", (int)parser->depth, "", "throw_after_capture", start, parser->pos);
    fprintf(stderr, "%*s\t%.*s\n", (int)parser->depth, "", (int)(parser->pos - start), parser->input + start);
  } else {
    fprintf(stderr, "%*sRule %s failed at position %zu\n", (int)parser->depth, "", "throw_after_capture", parser->pos);
  }
#endif
...
#define PGEN_PARSER_MT "pgen.t_test"
...
// cannot leak them.
static Parser *t_test_init(const char *input, lua_State *L) {
...
static void t_test_free(Parser *parser) {
  if (parser) {
    free(parser->caps);
    parser->caps = NULL;
  }
...
static int l_t_test_gc(lua_State *L) {
  t_test_free((Parser *)lua_touserdata(L, 1));
  return 0;
}

// Lua wrapper function
static int l_t_test_parse(lua_State *L) {
  ...
  Parser *parser = t_test_init(input, L);
...
static const struct luaL_Reg t_test_module[] = {
    {"parse", l_t_test_parse}, // Expose l_parsername_parse as "parse" in Lua
    {NULL, NULL}               // Sentinel
};
...
int luaopen_t_test(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_t_test_gc);
    lua_setfield(L, -2, "__gc");
  }
  lua_pop(L, 1);

  luaL_newlib(L, t_test_module); // Creates table and registers functions
  return 1;
}
#else
...
int luaopen_t_test(lua_State *L) {
  ...
  lua_newtable(L);
  luaL_register(L, NULL, t_test_module);
  return 1;
}
#endif
...
gcc -shared -o t_test.so -fPIC t_test.c `pkg-config --cflags --libs lua5.1`

To use in Lua:
local t_test = require "t_test"
local result = t_test.parse("your input string")
*/
