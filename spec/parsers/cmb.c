    ...
    { // Sequence with 3 patterns
      ...
      if (parser->success) {
        { // Capture Match Back "maybe"
          parser->success = pgen_cap_match_back(parser, 2);
          if (!parser->success) {
            PGEN_RECORD_FURTHEST(parser);
#ifdef PGEN_ERRORS
                      ...
                      { // Sequence with 3 patterns
                        ...
                        if (parser->success) {
                          { // Capture Match Back "eq"
                            parser->success = pgen_cap_match_back(parser, 1);
                            if (!parser->success) {
                              PGEN_RECORD_FURTHEST(parser);
#ifdef PGEN_ERRORS
  ...
  { // Sequence with 2 patterns
    ...
    if (parser->success) {
      { // Capture Match Back "x"
        parser->success = pgen_cap_match_back(parser, 3);
        if (!parser->success) {
          PGEN_RECORD_FURTHEST(parser);
#ifdef PGEN_ERRORS
...
#define PGEN_PARSER_MT "pgen.cmb"
...
static int l_cmb_gc(lua_State *L) {
  cmb_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
