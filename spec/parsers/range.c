...
#define PGEN_PARSER_MT "pgen.range"
...
static int l_range_gc(lua_State *L) {
  range_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_range(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_range_gc);
...
int luaopen_range(lua_State *L) {
  ...
  lua_newtable(L);
  luaL_register(L, NULL, range_module);
  return 1;
}
#endif
