...
#define PGEN_PARSER_MT "pgen.repeat_class"
...
static int l_repeat_class_gc(lua_State *L) {
  repeat_class_free((Parser *)lua_touserdata(L, 1));
  return 0;
}
...
int luaopen_repeat_class(lua_State *L) {
  if (luaL_newmetatable(L, PGEN_PARSER_MT)) {
    lua_pushcfunction(L, l_repeat_class_gc);
