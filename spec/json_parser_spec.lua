describe("json_parser", function()
  ...
  describe("labeled errors", function()
    it("reports expected_eof for trailing content", function()
      local result, label, pos = json_parser.parse('"valid" extra')
      assert.is_nil(result)
      assert.equal("expected_eof", label)
      assert.equal(9, pos)
    end)

    it("reports expected_closing_brace for unclosed object", function()
      local result, label, pos = json_parser.parse('{"key": "value"')
      assert.is_nil(result)
      assert.equal("expected_closing_brace", label)
      assert.equal(16, pos)
    end)

    it("reports expected_closing_bracket for unclosed array", function()
      local result, label, pos = json_parser.parse('[1, 2, 3')
      assert.is_nil(result)
      assert.equal("expected_closing_bracket", label)
      assert.equal(9, pos)
    end)

    it("reports expected_colon for missing colon in object", function()
      local result, label, pos = json_parser.parse('{"key" "value"}')
      assert.is_nil(result)
      assert.equal("expected_colon", label)
      assert.equal(8, pos)
    end)

    it("reports expected_value for missing value after colon", function()
      local result, label, pos = json_parser.parse('{"key": }')
      assert.is_nil(result)
      assert.equal("expected_value", label)
      assert.equal(9, pos)
    end)

    it("reports expected_member for trailing comma in object", function()
      local result, label, pos = json_parser.parse('{"key": "value",}')
      assert.is_nil(result)
      assert.equal("expected_member", label)
      assert.equal(17, pos)
    end)

    it("reports expected_array_element for trailing comma in array", function()
      local result, label, pos = json_parser.parse('[1, 2,]')
      assert.is_nil(result)
      assert.equal("expected_array_element", label)
      assert.equal(7, pos)
    end)

    it("reports expected_closing_quote for unterminated string", function()
      local result, label, pos = json_parser.parse('"unterminated')
      assert.is_nil(result)
      assert.equal("expected_closing_quote", label)
      assert.equal(14, pos)
    end)

    it("reports invalid_escape_sequence for bad escape", function()
      local result, label, pos = json_parser.parse('"bad\\qescape"')
      assert.is_nil(result)
      assert.equal("invalid_escape_sequence", label)
      assert.equal(6, pos)
    end)
  end)
end)
